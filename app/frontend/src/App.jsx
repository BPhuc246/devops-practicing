...
import { getTodos, createTodo, toggleTodo, deleteTodo, updateTodo, deleteCompleted } from './api';
...
export default function App() {
  ...
  useEffect(() => {
    getTodos()
      .then(setTodos)
      .catch(() => setError('Không thể kết nối server. Hãy chắc chắn backend đang chạy.'))
  ...
  const handleAdd = async (data) => {
    ...
    } catch { setError('Lỗi khi thêm công việc.'); }
  };
  ...
  const handleDelete = async (id) => {
    try {
      await deleteTodo(id);
      setTimeout(() => setTodos(prev => prev.filter(t => t.id !== id)), 250);
    } catch { setError('Lỗi khi xoá.'); }
  };
      ...
      <div className="max-w-xl mx-auto">
        ...
        <div className="mb-10">
          ...
          <h1 className="font-display text-4xl text-[var(--ink)] leading-tight">
            Hôm nay,<br />
            <em className="font-display italic text-[var(--rust)]">làm gì nhỉ?</em>
          </h1>
          ...
          {FILTERS.map(f => (
            ...
            >
              ...
              {f === 'Tất cả' && totalCount > 0 && (
                <span className="ml-1 font-mono text-[10px] opacity-60">({totalCount})</span>
              )}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-2 max-h-[480px] overflow-y-auto scrollbar-thin pr-0.5">
          {loading ? (
            <div className="py-16 text-center">
              ...
              <p className="text-sm text-[var(--muted)] font-body animate-pulse-soft">Đang tải...</p>
            </div>
          ) : filtered.length === 0 ? (
            <div className="py-16 text-center">
              <p className="text-3xl mb-3">
                {filter === 'Hoàn thành' ? '🎉' : filter === 'Đang làm' ? '✨' : '📝'}
              </p>
              <p className="text-sm text-[var(--muted)] font-body">
                {filter === 'Hoàn thành' ? 'Chưa có gì hoàn thành.' :
                 filter === 'Đang làm' ? 'Không có việc đang làm.' :
